 ...
 * @param {function} handlers.consumer consumer type to handle messages (e.g. retryOnce)
...
function createWorkers(handlers, config, options = {}) {
  ...
  function* _close(forceExit = true) {
    ...
    try {
      yield Array.from(workerChannels, ([channel, consumerTag]) => channel.cancel(consumerTag));
      ...
      logger.error(
        { workerName: configuration.workerName, err },
    ...
    if (workerConnection) {
      try {
        yield workerConnection.close(forceExit);
        logger.info(
          { workerName: configuration.workerName },
        ...
        logger.error(
          { workerName: configuration.workerName, error },
   ...
   * @param {function} consumer the consumer type
   ...
   * @private
   */
  function _getMessageConsumer({ channel, consumer, handle, validate, routingKey }) {
   ...
   *
   * @param {function} consumer message consumer type
   ...
   * @private
   */
  function* _handleMessage(consumer, handler, content, fields) {
    const { workerName, taskTimeout } = configuration;
    const childLogger = logger.child({ content, fields, options: configWithoutFuncs, workerName });

    try {
      const res = yield consumer(
        () => promisifyWithTimeout(handler(content, fields), workerName, taskTimeout),
        childLogger,
        fields
      );

      _emit(res ? TASK_COMPLETED : TASK_FAILED);
      return res;
    } catch (err) {
      childLogger.error({ err }, '[worker] Error handling message');
      _emit(TASK_FAILED);
      return true;
    }
  }
  ...
  function _subscribeToConnectionEvents(connection, workerName) {
    connection.on('close', errCode => {
      logger.info({ workerName, errCode }, '[AMQP] Connection closing, exiting');
      // The workerConnection isn't working anymore...
      workerConnection = null;
      workers.close();
    });
  ...
  function _subscribeToChannelEvents(channel,
    ...
    channel.on('close', () => {
      ...
      // Remove this channel because is closed
      workerChannels.delete(channel);
      workers.close();
    });
  }
 ...
 * @param   {function} handler.consumer the message consumer type
 ...
 * @private
 */
  function* _bindHandler({ routingKey, consumer, handle, validate }) {
    const {
      exchangeName,
      exchangeType,
      queueName,
    ...
    const channel = yield _createChannel(workerConnection, {
      exchangeName,
      exchangeType,
    ...
    yield channel.bindQueue(formattedQueueName, exchangeName, routingKey);
    const { consumerTag } = yield channel.consume(
      formattedQueueName,
      _getMessageConsumer({ channel, consumer, handle, validate, routingKey })
    );
    workerChannels.set(channel, consumerTag);
  }
}
