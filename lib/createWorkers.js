...
const HANDLE_MESSAGE_RESPONSE = Object.freeze({
  ACK: 'ack',
  NACK_REQUEUE: 'nack.requeue',
  NACK_DO_NOT_REQUEUE: 'nack.do.not.requeue'
});
...
function createWorkers(handlers, config, options = {}) {
  ...
  function _getMessageConsumer(channel, handle, validate, routingKey) {
    return function* _consumeMessage(message) {
      try {
        ...
        const handleMessageResponse = yield _handleMessage(
          handle,
          validatedContent || content,
          message.fields
        );
        if (handleMessageResponse === HANDLE_MESSAGE_RESPONSE.ACK) {
          return channel.ack(message);
        } else if (handleMessageResponse === HANDLE_MESSAGE_RESPONSE.NACK_REQUEUE) {
          return channel.nack(message);
        } else if (handleMessageResponse === HANDLE_MESSAGE_RESPONSE.NACK_DO_NOT_REQUEUE) {
          return channel.nack(message, false, false);
        }

        throw new Error('Bad handle message response');
      } catch (err) {
   ...
   * @returns {HANDLE_MESSAGE_RESPONSE} ACK if message should be acked,
   * NACK_REQUEUE if it should be nacked and requeued
   * NACK_DO_NOT_REQUEUE if it should be nacked but not requeued
   * @private
   */
  function* _handleMessage(handler, content, fields) {
    ...
    try {
      ...
      return HANDLE_MESSAGE_RESPONSE.ACK;
    } catch (err) {
      if (!fields.redelivered) {
        ...
        _emit(TASK_RETRIED);
        return HANDLE_MESSAGE_RESPONSE.NACK_REQUEUE;
      }
      logger.error(
        { err, content, options: configWithoutFuncs, workerName },
      ...
      _emit(TASK_FAILED);
      return HANDLE_MESSAGE_RESPONSE.NACK_DO_NOT_REQUEUE;
    }
  }
  ...
  function* _createChannel(connection,
    ...
    logger.info(
      { exchangeName, queueName, queueOptions, channelPrefetch, workerName },
      '[AMQP] create channel'
    );
